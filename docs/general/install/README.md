...
```hcl
module "synthetics" {
    source = "github.com/pbs/terraform-synthetics-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/synthetics'
gh -R 'pbs/terraform-synthetics-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/synthetics'
rm -f release.tar.gz
```
...
```bash
git remote add -f synthetics git@github.com:pbs/terraform-synthetics-module.git
git subtree add --prefix terraform/modules/synthetics synthetics main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/synthetics synthetics $REF --squash
```
